...
export interface DirectoryItem { id:number; slug?:string; brand?:string; model?:string; year?:number; plate_number?:string; color?:string|null; category?:string; passenger_capacity?:number; luggage_capacity?:number; transmission?:string|null; air_conditioning?:boolean; wifi?:boolean; child_seat_available?:boolean; base_price_minor?:number; price_per_km_minor?:number; price_per_hour_minor?:number; currency?:string; first_name?:string; last_name?:string; phone?:string; email?:string|null; locale?:string; languages?:string[]; experience_years?:number; license_number?:string; rating?:number|null; preferred_car_id?:number|null; car_ids?:number[]; cars?:Array<{id:number;name:string;plate_number:string}>; telegram_connected?:boolean; profile_image?:Media|null; active:boolean; featured?:boolean; available_for_booking?:boolean; is_fallback?:boolean; translations?:Array<{locale:string;name?:string;title?:string}>; cover_image?:Media|null }
...
export interface DriverAdminInput { first_name:string; last_name:string; phone:string; email:string; password?:string; locale:'en'|'ru'|'hy'; languages:string[]; experience_years:number; license_number:string; rating:number|null; active:boolean; preferred_car_id:number|null; car_ids:number[] }
...
export interface TourItineraryInput { destination_id:number; day_number:number; duration_minutes:number|null; optional:boolean; notes:string|null }
export interface AdminTourItineraryStop extends TourItineraryInput { id:number; stop_order:number; destination:{id:number;slug:string;name:string|null}|null }
export interface TourAdminInput { category_id:number|null; slug:string; duration_minutes:number; approximate_distance_km:number|null; starting_price_minor:number; currency:Currency; pricing_type:PricingType; format:TourFormat; start_time:string|null; meeting_point:string|null; active:boolean; featured:boolean; max_passengers:number|null; pickup_available:boolean; dropoff_available:boolean; free_cancellation_hours:number; sort_order:number; translations:Array<AdminTranslation&{title:string}>; itinerary:TourItineraryInput[] }
export interface AdminTour extends Omit<TourAdminInput,'itinerary'> { id:number; itinerary:AdminTourItineraryStop[]; cover_image:Media|null; gallery?:Media[]; video:Media|null }
...
export interface CatalogResponse<T> { data:T[]; current_page:number; last_page:number; total:number }
...
export const adminApi = {
  ...
  createDriver:async(input:DriverAdminInput):Promise<DirectoryItem>=>(await apiClient.post<ApiEnvelope<DirectoryItem>>('/admin/directory/drivers',input)).data.data,
  updateDriver:async(id:number,input:Partial<DriverAdminInput>):Promise<DirectoryItem>=>(await apiClient.patch<ApiEnvelope<DirectoryItem>>(`/admin/directory/drivers/${id}`,input)).data.data,
  deleteDriver:async(id:number):Promise<void>=>{await apiClient.delete(`/admin/directory/drivers/${id}`)},
  tours:async():Promise<CatalogResponse<AdminTour>>=>(await apiClient.get<CatalogResponse<AdminTour>>('/admin/directory/tours',{params:{per_page:100}})).data,
  createTour:async(input:TourAdminInput):Promise<AdminTour>=>(await apiClient.post<ApiEnvelope<AdminTour>>('/admin/directory/tours',input)).data.data,
  updateTour:async(id:number,input:Partial<TourAdminInput>):Promise<AdminTour>=>(await apiClient.patch<ApiEnvelope<AdminTour>>(`/admin/directory/tours/${id}`,input)).data.data,
  deleteTour:async(id:number):Promise<void>=>{await apiClient.delete(`/admin/directory/tours/${id}`)},
  destinations:async():Promise<CatalogResponse<AdminDestination>>=>(await apiClient.get<CatalogResponse<AdminDestination>>('/admin/directory/destinations',{params:{per_page:100}})).data,
  createDestination:async(input:DestinationAdminInput):Promise<AdminDestination>=>(await apiClient.post<ApiEnvelope<AdminDestination>>('/admin/directory/destinations',input)).data.data,
  ...
  tourMedia:async(id:number,slug:string):Promise<Media[]>=>{try{return (await apiClient.get<ApiEnvelope<Media[]>>(`/admin/media/tours/${id}`)).data.data}catch{const tour=(await apiClient.get<ApiEnvelope<Tour>>(`/tours/${slug}`,{params:{locale:'en'}})).data.data;const gallery=(tour.gallery??[]).map((image)=>({...image,collection:'gallery' as const}));const images=tour.cover_image&&!gallery.some((image)=>image.id===tour.cover_image?.id)?[{...tour.cover_image,collection:'cover' as const},...gallery]:gallery;return tour.video?[...images,{...tour.video,collection:'video' as const}]:images}},
  uploadMedia:async(type:DirectoryType,id:number,file:File,collection?:'cover'|'gallery'|'profile'|'video',sortOrder?:number):Promise<Media>=>{const form=new FormData();form.append('file',file);form.append('collection',collection??(type==='drivers'?'profile':'cover'));if(sortOrder!==undefined)form.append('sort_order',String(sortOrder));return (await apiClient.post<ApiEnvelope<Media>>(`/admin/media/${type}/${id}`,form)).data.data},
