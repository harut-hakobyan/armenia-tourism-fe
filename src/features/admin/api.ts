...
export interface TourAdminInput { category_id:number|null; slug:string; duration_minutes:number; approximate_distance_km:number|null; starting_price_minor:number; currency:Currency; pricing_type:PricingType; format:TourFormat; start_time:string|null; meeting_point:string|null; active:boolean; featured:boolean; max_passengers:number|null; pickup_available:boolean; dropoff_available:boolean; free_cancellation_hours:number; sort_order:number; translations:Array<AdminTranslation&{title:string}> }
export interface AdminTour extends TourAdminInput { id:number; cover_image:Media|null; gallery?:Media[]; video:Media|null }
