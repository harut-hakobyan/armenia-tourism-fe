...
const values = [
  {
    icon: Users,
    title: "content.localDrivers",
    text: "content.localDriversText",
  },
  {
    icon: ShieldCheck,
    title: "content.dependable",
    text: "content.dependableText",
  },
  {
    icon: Sparkles,
    title: "content.madeAroundYou",
    text: "content.madeAroundYouText",
  },
] as const;
      ...
      <div className="mt-14 grid gap-5 md:grid-cols-3">
        {values.map(({ icon: Icon, title, text }) => (
          <div key={title} className="rounded-3xl bg-white p-7 shadow-soft">
            <Icon className="text-apricot" />
            <h2 className="mt-4 text-xl font-bold">{t(title)}</h2>
            <p className="mt-3 leading-7 text-ink/60">{t(text)}</p>
          </div>
        ))}
      </div>
    </Container>
    ...
    <Container className="py-16 sm:py-24">
      <div className="mx-auto max-w-2xl">
        <div className="text-center">
          <MessageCircle className="mx-auto size-12 text-apricot" />
          <h1 className="text-display mt-5 text-6xl">
...
export function FaqPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () => contentApi.faqs(i18n.language),
  });
  ...
  return (
    <Container className="py-16 sm:py-24">
      <div className="mx-auto max-w-3xl">
        <p className="text-sm font-bold uppercase tracking-[.2em] text-apricot">
          {t("content.faqEyebrow")}
        </p>
        <h1 className="text-display mt-3 text-6xl">{t("content.faqTitle")}</h1>
        <div className="mt-10 divide-y divide-black/8 rounded-3xl bg-white px-6 shadow-soft">
          {query.data?.map((item) => (
            <details key={item.id} className="group py-6">
              <summary className="cursor-pointer list-none pr-8 text-lg font-bold">
                {item.question}
              </summary>
              <p className="mt-3 leading-7 text-ink/60">{item.answer}</p>
            </details>
          ))}
        </div>
      </div>
