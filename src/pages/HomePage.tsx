...
export function HomePage() {
  ...
  return <>
    ...
    <section className="py-20 sm:py-28"><Container><div className="mb-10 flex items-end justify-between gap-5"><div><p className="text-sm font-bold uppercase tracking-widest text-apricot">Curated journeys</p><h2 className="text-display mt-2 text-4xl sm:text-5xl">Popular private tours</h2></div><Link to="/tours" className="hidden font-semibold text-forest sm:block">View all →</Link></div>
      {tours.isError ? <QueryError retry={() => void tours.refetch()} /> : <div className="grid gap-6 md:grid-cols-3">{tours.isPending ? [1, 2, 3].map((item) => <div key={item} className="h-96 animate-pulse rounded-3xl bg-sand/60" />) : tours.data.data.map((tour) => <Link to={`/tours/${tour.slug}`} key={tour.id} className="group overflow-hidden rounded-3xl bg-white shadow-soft"><div className="h-52 bg-[linear-gradient(145deg,#244b3a,#d87948)] transition group-hover:scale-[1.02]" /> <div className="p-6"><p className="text-sm font-semibold uppercase tracking-wider text-apricot">{tour.category?.name}</p><h3 className="text-display mt-2 text-2xl">{tour.title}</h3><p className="mt-4 text-sm text-ink/60">{Math.round(tour.duration_minutes / 60)} hours · from {formatMoney(tour.starting_price.amount_minor, tour.starting_price.currency, i18n.language)}</p></div></Link>)}</div>}
    </Container></section>
