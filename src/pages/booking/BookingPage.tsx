...
export function BookingPage() {
  ...
  const initialService = (params.get('service') ?? draft?.service_type ?? 'tour') as ServiceType
  const [step, setStep] = useState(1)
  const [error, setError] = useState<string|null>(null)
  const [form, setForm] = useState({service:initialService,tour:Number(params.get('tour')??draft?.tour_id??0),departure:Number(params.get('departure')??0),car:Number(params.get('car')??draft?.car_id??0),date:'',time:'09:00',passengers:2,pickup:'',name:'',email:'',phone:'',whatsapp:'',notes:'',duration:draft?.duration_minutes??480})
  ...
  const group = selectedTour?.format === 'group'
  ...
  const estimate = useMutation({mutationFn:()=>{
    ...
    if(form.service==='tour') return estimateApi.tour({...contact,tour_id:form.tour,booking_date:effectiveDate,...(group?{group_tour_departure_id:form.departure}:{car_id:form.car})})
    const base={...contact,car_id:form.car}
    if(form.service==='private_driver') return estimateApi.privateDriver({...base,duration_minutes:form.duration})
    const route=draft?.route_points??defaultRoute
    return form.service==='airport_transfer'?estimateApi.transfer({...base,route_points:route}):estimateApi.customTrip({...base,route_points:route})
  }})
  ...
  function chooseTour(tourId: number) { setForm(current => ({...current, tour: tourId, departure: 0, car: 0, date: ''})) }
  const journeyReady = form.service === 'tour'
    ? Boolean(form.tour && (group ? form.departure : form.date && form.car))
    : Boolean(form.date && form.car)

  async function review() {
    setError(null)
    if (!journeyReady) { setError(group ? 'Choose an available group departure.' : 'Choose a service, date, and car first.'); return }
  ...
  async function submit() {
    setError(null)
    try {
      const routeService=form.service==='airport_transfer'||form.service==='custom_trip'
      const result=await create.mutateAsync({
        idempotency_key:crypto.randomUUID(),service_type:form.service,
        ...(form.tour?{tour_id:form.tour}:{}),...(group?{group_tour_departure_id:form.departure}:{car_id:form.car}),
        ...
        ...(form.service==='private_driver'?{duration_minutes:form.duration}:{}),...(routeService?{route_points:draft?.route_points??defaultRoute}:{}),
  ...
  return <Container className="py-12 sm:py-20"><div className="mx-auto max-w-4xl"><p className="text-sm font-bold uppercase tracking-[.2em] text-apricot">Book securely</p><h1 className="text-display mt-2 text-5xl">Your Armenia journey</h1><div className="mt-8 grid grid-cols-3 gap-2">{labels.map((label,index)=><div key={label} className={`rounded-xl px-3 py-3 text-center text-xs font-bold ${step>=index+1?'bg-forest text-white':'bg-stone text-ink/40'}`}>{step>index+1?<Check className="mx-auto size-4"/>:label}</div>)}</div>
    <div className="mt-8 rounded-3xl bg-white p-6 shadow-soft sm:p-8">
      {step===1&&<div className="grid gap-5 sm:grid-cols-2"><label className="text-sm font-semibold sm:col-span-2">Service<select value={form.service} onChange={e=>update('service',e.target.value as ServiceType)} className="mt-2 min-h-12 w-full rounded-xl border border-black/10 px-4"><option value="tour">Tour — private or group</option><option value="airport_transfer">Airport transfer</option><option value="private_driver">Private driver</option><option value="custom_trip">Custom trip</option></select></label>
        {form.service==='tour'&&<label className="text-sm font-semibold sm:col-span-2">Tour<select value={form.tour} onChange={e=>chooseTour(Number(e.target.value))} className="mt-2 min-h-12 w-full rounded-xl border border-black/10 px-4"><option value="0">Choose a tour</option>{tours.data?.data.map(t=><option key={t.id} value={t.id}>{t.format === 'group' ? 'Group' : 'Private'} · {t.title}</option>)}</select></label>}
        ...
        {form.service==='private_driver'&&<label className="text-sm font-semibold">Duration<select value={form.duration} onChange={e=>update('duration',Number(e.target.value))} className="mt-2 min-h-12 w-full rounded-xl border border-black/10 px-4"><option value="240">4 hours</option><option value="480">8 hours</option><option value="720">12 hours</option><option value="1440">Full day</option></select></label>}
