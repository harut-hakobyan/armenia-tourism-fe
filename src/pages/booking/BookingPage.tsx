...
export function BookingPage() {
  ...
  const selectedPassengers = validPassengerCount(params.get('passengers') ?? draft?.passengers ?? draft?.estimate?.passengers)
  const [form, setForm] = useState({service:initialService,tour:Number(params.get('tour')??draft?.tour_id??0),date:'',time:'09:00',passengers:initialChoice === 'group_tour' ? 1 : selectedPassengers,pickup:'',name:'',email:'',phone:'',whatsapp:'',notes:''})
  ...
  const effectiveBookingChoice: BookingChoice = serviceLocked && selectedTour
    ...
    : bookingChoice
  const serviceLabel = effectiveBookingChoice === 'group_tour' ? 'Group tour' : effectiveBookingChoice === 'private_tour' ? 'Private tour' : 'Custom trip'
  ...
  const effectiveTime = group ? selectedTour?.start_time ?? '09:00' : form.time
  const effectivePickup = group ? selectedTour?.meeting_point ?? '' : form.pickup

  const estimate = useMutation({mutationFn:()=>{
    const contact = {passengers:effectivePassengers,...(form.email?{customer_email:form.email}:{})}
    if(form.service==='tour') return estimateApi.tour({...contact,tour_id:form.tour,booking_date:effectiveDate,car_id:automaticCarId})
  ...
  function chooseTour(tourId: number) { setForm(current => ({...current, tour: tourId, date: ''})) }
  function chooseService(choice: BookingChoice) {
    setBookingChoice(choice)
    setForm((current) => ({ ...current, service: choice === 'custom_trip' ? 'custom_trip' : 'tour', tour: 0, date: '', passengers: choice === 'group_tour' ? 1 : current.passengers }))
  }
  const journeyReady = form.service === 'tour'
    ? Boolean(form.tour && form.date && automaticCarId && (!group || effectivePickup))
    : Boolean(form.date && automaticCarId)

  async function review() {
    setError(null)
    if (!journeyReady) { setError(form.service === 'custom_trip' ? 'Choose a date first.' : 'Choose a tour and date first.'); return }
  ...
  async function submit() {
    ...
    try {
      ...
      const result=await create.mutateAsync({
        ...
        ...(form.tour?{tour_id:form.tour}:{}),car_id:automaticCarId,
        booking_date:effectiveDate,pickup_time:effectiveTime,passengers:effectivePassengers,pickup_address:effectivePickup,
  ...
  return <Container className="py-12 sm:py-20"><div className="mx-auto max-w-4xl"><p className="text-sm font-bold uppercase tracking-[.2em] text-apricot">Book securely</p><h1 className="text-display mt-2 text-5xl">Your Armenia journey</h1><div aria-label={`Booking progress: step ${step} of ${labels.length}`} className="mt-8 flex items-center rounded-2xl border border-black/6 bg-white p-2 shadow-[0_10px_35px_rgb(23_35_29/0.06)] sm:p-3">{labels.map((label,index)=>{const number=index+1;const complete=step>number;const active=step===number;return <div key={label} className="contents"><div aria-current={active?'step':undefined} className={`flex min-w-0 flex-1 items-center gap-2 rounded-xl px-2 py-2.5 transition sm:gap-3 sm:px-4 ${active?'bg-forest text-white shadow-md shadow-forest/15':complete?'text-forest':'text-ink/40'}`}><span className={`grid size-7 shrink-0 place-items-center rounded-full text-xs font-extrabold sm:size-8 ${active?'bg-white text-forest':complete?'bg-forest text-white':'bg-stone text-ink/45'}`}>{complete?<Check className="size-4"/>:number}</span><span className="truncate text-xs font-bold sm:text-sm">{label}</span></div>{index<labels.length-1&&<ChevronRight aria-hidden="true" className={`mx-0.5 size-4 shrink-0 sm:mx-2 sm:size-5 ${complete?'text-forest':'text-ink/20'}`}/>}</div>})}</div>
    <div className="mt-8 rounded-3xl bg-white p-6 shadow-soft sm:p-8">
      {step===1&&<div className="grid gap-5 sm:grid-cols-2">{serviceLocked ? <div className="rounded-2xl bg-stone p-4 text-sm sm:col-span-2"><strong>Service:</strong> {serviceLabel}</div> : <label className="text-sm font-semibold sm:col-span-2">Service<select value={effectiveBookingChoice} onChange={e=>chooseService(e.target.value as BookingChoice)} className="mt-2 min-h-12 w-full rounded-xl border border-black/10 px-4"><option value="group_tour">Group tour</option><option value="private_tour">Private tour</option><option value="custom_trip">Custom trip</option></select></label>}
        ...
        <label className={`min-w-0 text-sm font-semibold ${group ? 'sm:col-span-2' : ''}`}>Date
        ...
        {group
          ? <label className="text-sm font-semibold sm:col-span-2">Passengers<NumericInput required min={1} max={Math.min(selectedTour?.max_passengers??20,20)} value={effectivePassengers} onValueChange={(value)=>{if(value!==null)update('passengers',value)}} className="mt-2 min-h-12 w-full rounded-xl border border-black/10 px-4"/></label>
          : <div className="rounded-2xl bg-stone p-4 text-sm sm:col-span-2"><strong>Passengers:</strong> {form.passengers}</div>}
        ...
        {group&&selectedTour&&<div className="grid gap-3 sm:col-span-2 sm:grid-cols-2"><div className="rounded-2xl bg-stone p-4 text-sm"><strong>Start time:</strong> {effectiveTime}</div><div className="rounded-2xl bg-stone p-4 text-sm"><strong>Meeting place:</strong> {effectivePickup||'To be confirmed'}</div></div>}
