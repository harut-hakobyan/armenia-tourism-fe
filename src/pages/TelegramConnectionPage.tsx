...
export function TelegramConnectionPage() {
  ...
  const data = status.data?.connected ? status.data : link.data ?? preferences.data ?? status.data
  ...
  return <div className="max-w-3xl">
    ...
    <div className="mt-7 rounded-3xl bg-white p-6 shadow-sm">
      ...
      {!data?.connected && !link.data && <Button onClick={() => link.mutate()} disabled={link.isPending || !data?.configured} className="mt-7"><Link2 className="mr-2 size-4" />{link.isPending ? 'Creating secure link…' : 'Create connection link'}</Button>}
