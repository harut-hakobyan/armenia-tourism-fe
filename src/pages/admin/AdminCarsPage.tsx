...
function fromItem(item: DirectoryItem): CarForm {
  return {
    ...
    child_seat_available: item.child_seat_available ?? false,
    active: item.active,
    available_for_booking: item.available_for_booking ?? true,
  };
}
...
export function AdminCarsPage() {
  ...
  const query = useQuery({
    queryKey: ["admin", "directory", "cars"],
    queryFn: () => adminApi.directory("cars"),
  });
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return editing
        ? adminApi.updateDirectory("cars", editing, input)
        : adminApi.createCar(input);
    },
    onSuccess: async () => {
      await refresh();
      setOpen(false);
  ...
  function field<K extends keyof CarForm>(key: K, value: CarForm[K]) {
  ...
  function edit(item: DirectoryItem) {
    setEditing(item.id);
    ...
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold">Cars</h1>
        </div>
        <Button onClick={add}>
        ...
        >
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <NumberField
              label="Year"
              value={form.year}
              onChange={(value) => field("year", value)}
              min={1980}
              required
            />
            <Text
              ...
              onChange={(value) => field("plate_number", value)}
              required
            />
            <Text
              label="Color"
            ...
            <label className="text-sm font-semibold">
              ...
              <select
                value={form.category}
                onChange={(event) =>
                  field("category", event.target.value as CarForm["category"])
                }
            ...
            <NumberField
              ...
              onChange={(value) => field("luggage_capacity", value)}
              min={0}
              required
            />
            <Text
              ...
              onChange={(value) => field("transmission", value)}
            />
          </div>
          <div className="mt-6 flex flex-wrap gap-5">
            {(
              [
                ...
                ["wifi", "Wi-Fi"],
                ["child_seat_available", "Child seat"],
                ["active", "Active"],
                ["available_for_booking", "Available for assignment"],
              ] as const
            ).map(([key, label]) => (
              <label
                key={key}
          ...
          <Button type="submit" disabled={save.isPending} className="mt-7">
            {save.isPending ? "Saving…" : "Save car"}
          </Button>
        </form>
      )}
      <div className="mt-7 overflow-hidden rounded-2xl bg-white shadow-sm">
        {query.data?.data.map((item) => (
          <div
            key={item.id}
            className="flex flex-wrap items-center justify-between gap-5 border-b border-black/5 px-5 py-4"
          >
            <div className="flex items-center gap-4">
              {item.cover_image ? (
                ...
                <div className="grid size-16 place-items-center rounded-xl bg-stone text-ink/30">
                  <ImagePlus />
                </div>
              )}
              <div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {item.cover_image && (
                <button
                  onClick={() => {
                    if (window.confirm("Remove this car image?"))
                      removeImage.mutate(item.cover_image!.id);
                  }}
                  className="rounded-full bg-red-50 px-3 py-2 text-xs font-bold text-danger"
                >
                  Remove image
                </button>
              )}
              ...
              <button
                onClick={() => {
                  if (window.confirm(`Remove ${item.brand} ${item.model}?`))
                    remove.mutate(item.id);
                }}
                className="grid size-9 place-items-center rounded-full bg-red-50 text-danger"
