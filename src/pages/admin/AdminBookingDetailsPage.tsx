  ...
  return <div>
    ...
    <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_360px]">
      <div className="space-y-6">
        <section className="rounded-3xl bg-white p-6 shadow-sm"><h2 className="font-bold">Trip details</h2><dl className="mt-5 grid gap-5 sm:grid-cols-2"><Item label="Service" value={current.service_type.replaceAll('_', ' ')} /><Item label="Date & time" value={`${current.booking_date} · ${current.pickup_time.slice(0, 5)}`} /><Item label="Pickup" value={current.pickup.address} /><Item label="Passengers" value={String(current.passengers)} /><Item label="Attendance" value={`${current.attendance.checked_in_passengers} / ${current.passengers} checked in`} /><Item label="Requested vehicle type" value={current.requested_car_type?.replaceAll('_', ' ') ?? '—'} />{current.car&&<Item label="Reserved car" value={current.car.name} />}<Item label="Total" value={formatMoney(current.price.total_minor, current.price.currency)} /></dl></section>
