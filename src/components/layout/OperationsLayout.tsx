import { BookOpen, CalendarDays, CarFront, Gauge, History, LogOut, Mail, Map, MapPin, MessageCircle, Mountain, QrCode, Settings, Star, Tag, UsersRound } from 'lucide-react'
import { NavLink, Outlet } from 'react-router-dom'
import { cn } from '@/lib/cn'
import { useAuth } from '@/features/auth/auth-context'

const adminLinks = [['/admin', 'Dashboard', Gauge], ['/admin/check-in', 'QR check-in', QrCode], ['/admin/bookings', 'Bookings', CalendarDays], ['/admin/calendar', 'Calendar', CalendarDays], ['/admin/tours', 'Tours', Map], ['/admin/destinations', 'Destinations', MapPin], ['/admin/cars', 'Cars', CarFront], ['/admin/drivers', 'Drivers', UsersRound], ['/admin/customers', 'Customers', UsersRound], ['/admin/reviews', 'Reviews', Star], ['/admin/promo-codes', 'Promo codes', Tag], ['/admin/faqs', 'FAQs', BookOpen], ['/admin/inquiries', 'Inquiries', Mail], ['/admin/telegram', 'Telegram', MessageCircle]] as const
...
export function OperationsLayout({ driver = false }: { driver?: boolean }) {
  ...
  const links = driver ? [['/driver', 'My trips', CalendarDays], ['/driver/check-in', 'QR check-in', QrCode], ['/driver/telegram', 'Telegram', MessageCircle]] as const : user?.role==='admin' ? [...adminLinks,...superAdminLinks] : adminLinks
