...
const adminLinks = [
  ...
  ["/admin/check-in", "QR check-in", QrCode],
  ...
  ["/admin/promo-codes", "Promo codes", Tag],
  ["/admin/faqs", "FAQs", BookOpen],
  ["/admin/inquiries", "Inquiries", Mail],
  ["/admin/telegram", "Telegram", MessageCircle],
] as const;
...
export function OperationsLayout({ driver = false }: { driver?: boolean }) {
  ...
  const links = driver
    ? ([
        ["/driver", "My trips", CalendarDays],
        ["/driver/check-in", "QR check-in", QrCode],
        ["/driver/telegram", "Telegram", MessageCircle],
      ] as const)
    : user?.role === "admin"
      ? [...adminLinks, ...superAdminLinks]
      : adminLinks;
  return (
    <div className="min-h-screen bg-[#f4f5f2] lg:grid lg:grid-cols-[260px_1fr]">
      <aside className="border-b border-black/5 bg-ink p-5 text-white lg:min-h-screen lg:border-b-0">
        <div className="flex items-center gap-3 font-bold">
          <Mountain className="text-apricot-light" />
          Armenia Operations
        </div>
        <nav className="mt-7 flex gap-2 overflow-x-auto lg:grid">
          {links.map(([to, label, Icon]) => (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  "flex shrink-0 items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-white/70 hover:bg-white/8 hover:text-white",
      ...
      <section>
        <header className="flex min-h-18 items-center justify-between border-b border-black/5 bg-white px-5 sm:px-8">
          <div>
            <p className="text-sm text-black/50">Signed in as</p>
